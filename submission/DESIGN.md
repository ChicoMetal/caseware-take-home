...
- **Input validation:** Domain model records at the external trust boundary (`UserContext`, `UpdateDecision`) use compact constructors with `Objects.requireNonNull` for required fields and explicit range checks (e.g., `targetVersion > 0`). Invalid input is rejected at the domain boundary before reaching any business logic. Internal records (e.g., `EngagementRecord`, `ChangeSummary`) are populated by trusted infrastructure code and do not duplicate validation.
